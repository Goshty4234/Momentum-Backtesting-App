...
def single_backtest(config, sim_index, reindexed_data, _cache_version="v2_daily_allocations"):
    ...
    if first_rebalance_strategy == "momentum_window_complete" and use_momentum and momentum_windows:
        try:
            ...
            filtered_index = sim_index[sim_index >= momentum_completion_date]
            momentum_completion_trading_day = filtered_index.iloc[0] if len(filtered_index) > 0 else sim_index[-1]
    ...
    if 'strategy_comparison_all_results' in st.session_state and st.session_state.strategy_comparison_all_results:
        ...
        if selected_portfolio_detail:
            ...
            if selected_portfolio_detail in st.session_state.strategy_comparison_all_metrics:
                ...
                # Check if metrics are already processed and cached
                cache_key = f"processed_metrics_v2_{selected_portfolio_detail}"
                metrics_records = st.session_state.get(cache_key)
                
                if metrics_records is None:
                    # Process metrics data once and cache it
                    metrics_records = []
                ...
                if metrics_records:
                    ...
                    # Cache the processed metrics for future use
                    st.session_state[cache_key] = metrics_records

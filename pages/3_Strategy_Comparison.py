...
if 'strategy_special_tickers_expanded' not in st.session_state:
    st.session_state.strategy_special_tickers_expanded = False

with st.sidebar.expander("🎯 Special Long-Term Tickers", expanded=st.session_state.strategy_special_tickers_expanded):
    ...
    with col1:
        ...
        for name, (alias, ticker) in stock_mapping.items():
            if st.button(f"➕ {name}", key=f"add_stock_{ticker}", help=f"Add {alias} → {ticker}"):
                ...
                st.session_state.strategy_special_tickers_expanded = True
                st.rerun()

...
def generate_simple_pdf_report(custom_name=""):
    ...
    try:
        ...
        heading_style = ParagraphStyle(
            ...
            fontSize=16,  # Back to original size to prevent overlap
            spaceAfter=20,
                        ...
                        if trace.type == 'table':
                            ...
                            if hasattr(trace, 'cells') and trace.cells and hasattr(trace.cells, 'values'):
                                ...
                                if cell_data and len(cell_data) > 0:
                                    ...
                                    # Create table with smart column widths for statistics table - WIDER TABLE
                                    page_width = 8.2*inch  # Increased from 8.0 to 8.2 inches for maximum width usage
                                    ...
                                    if len(headers) > 8:  # If we have many columns, use optimized widths
                                        # Portfolio column: increased from 2.0 to 2.1 inches for better text wrapping
                ...
                if today_weights:
                    ...
                    try:
                        ...
                        if labels_today and vals_today:
                            ...
                            # Create pie chart without percentage labels (we'll add them under tickers)
                            wedges_target, texts_target, autotexts_target = ax_target.pie(vals_today, autopct='', 
                                                                                         startangle=90)
                            ...
                            # Add minimal separation between timer and allocation table
                            story.append(Spacer(1, 3))
                            
                            # Don't force a page break - keep timer and allocation on same page
                            # story.append(PageBreak())
                            
                            # Now add the allocation table
                                            ...
                                            if trace.type == 'table':
                                                ...
                                                if hasattr(trace, 'cells') and trace.cells and hasattr(trace.cells, 'values'):
                                                    ...
                                                    if cell_data and len(cell_data) > 0:
                                                        ...
                                                        page_width = 7.5*inch
                                                        # Asset column gets less space, allocation columns get more
                                                        if len(headers) >= 6:  # Full allocation table
                                                            col_widths = [1.8*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch]
                                                        else:  # Simple allocation table
...
def calculate_next_rebalance_date(rebalancing_frequency, last_rebalance_date):
    ...
    if last_rebalance_date and hasattr(last_rebalance_date, 'to_pydatetime'):
...
def single_backtest(config, sim_index, reindexed_data):
    ...
    if not use_momentum:
        ...
        # Apply maximum allocation filter for non-momentum strategies
        use_max_allocation = config.get('use_max_allocation', False)
        max_allocation_percent = config.get('max_allocation_percent', 10.0)
...
def bulk_delete_portfolios_callback(portfolio_names_to_delete):
    ...
    st.session_state.portfolio_checkboxes = {}
    ...
    # Set success message
    st.session_state.bulk_delete_success = f"Successfully deleted {deleted_count} portfolio(s)!"
...
def paste_json_callback():
    try:
        ...
        st.session_state['multi_backtest_active_use_threshold'] = multi_backtest_config.get('use_minimal_threshold', False)
        st.session_state['multi_backtest_active_threshold_percent'] = multi_backtest_config.get('minimal_threshold_percent', 2.0)
        
        # Update session state for maximum allocation settings
        st.session_state['multi_backtest_active_use_max_allocation'] = multi_backtest_config.get('use_max_allocation', False)
        st.session_state['multi_backtest_active_max_allocation_percent'] = multi_backtest_config.get('max_allocation_percent', 10.0)
        ...
        # Re-update session state for threshold and maximum allocation settings after sync (to ensure they're not overridden)
        st.session_state['multi_backtest_active_use_threshold'] = multi_backtest_config.get('use_minimal_threshold', False)
        st.session_state['multi_backtest_active_threshold_percent'] = multi_backtest_config.get('minimal_threshold_percent', 2.0)
        st.session_state['multi_backtest_active_use_max_allocation'] = multi_backtest_config.get('use_max_allocation', False)
        st.session_state['multi_backtest_active_max_allocation_percent'] = multi_backtest_config.get('max_allocation_percent', 10.0)
...
def update_active_portfolio_index():
    ...
    if portfolio_configs and st.session_state.multi_backtest_active_portfolio_index is not None:
        ...
        if 'use_max_allocation' not in active_portfolio:
            active_portfolio['use_max_allocation'] = False
        if 'max_allocation_percent' not in active_portfolio:
            active_portfolio['max_allocation_percent'] = 10.0
...
def generate_portfolio_name_with_threshold(portfolio):
    """
    Generate a portfolio name that includes threshold information in the format:
    Base Name (Rebalancing Frequency - Momentum Strategy : When momentum not all negative and When momentum all negative - Include Beta in weighting - Include Volatility in weighting - Min Threshold)
    """
    base_name = portfolio.get('name', 'Portfolio')
    
    # Remove existing threshold info if present to avoid duplication
    if ' - Min ' in base_name:
        base_name = base_name.split(' - Min ')[0]
    
    # Build name parts
    name_parts = []
    
    # Rebalancing frequency
    rebal_freq = portfolio.get('rebalancing_frequency', 'Annually')
    name_parts.append(rebal_freq)
    
    # Momentum strategy
    if portfolio.get('use_momentum', False):
        momentum_strategy = portfolio.get('momentum_strategy', 'Classic')
        if momentum_strategy == 'Classic':
            name_parts.append("Momentum : Classic")
        elif momentum_strategy == 'Relative Momentum':
            name_parts.append("Momentum : Relative")
        
        # Negative strategy
        negative_strategy = portfolio.get('negative_momentum_strategy', 'Cash')
        if negative_strategy == 'Cash':
            name_parts.append("and Cash")
        elif negative_strategy == 'Equal weight':
            name_parts.append("and Equal Weight")
        elif negative_strategy == 'Relative momentum':
            name_parts.append("and Relative")
        
        # Beta and Volatility
        if portfolio.get('calc_beta', False):
            name_parts.append("- Beta")
        if portfolio.get('calc_volatility', False):
            name_parts.append("- Volatility")
    else:
        name_parts.append("No Momentum")
    
    # Add threshold information
    if portfolio.get('use_minimal_threshold', False):
        threshold_percent = portfolio.get('minimal_threshold_percent', 2.0)
        name_parts.append(f"- Min {threshold_percent:.2f}%")
    else:
        name_parts.append("- Min Disabled")
    
    # Add maximum allocation information
    if portfolio.get('use_max_allocation', False):
        max_allocation_percent = portfolio.get('max_allocation_percent', 10.0)
        name_parts.append(f"- Max {max_allocation_percent:.2f}%")
    else:
        name_parts.append("- Max Disabled")
    
    # Create the full name
    full_name = f"{base_name} ({' '.join(name_parts)})"
    return full_name
...
def sync_date_widgets_with_portfolio():
    ...
    if st.session_state.multi_backtest_active_portfolio_index is not None:
        ...
        # Sync maximum allocation settings
        st.session_state["multi_backtest_active_use_max_allocation"] = portfolio.get('use_max_allocation', False)
        st.session_state["multi_backtest_active_max_allocation_percent"] = portfolio.get('max_allocation_percent', 10.0)
...
if len(st.session_state.multi_backtest_portfolio_configs) > 1:
    ...
    if "selected_portfolios_for_deletion" not in st.session_state:
        st.session_state.selected_portfolios_for_deletion = []
    ...
    with st.sidebar.expander("📋 Manage Multiple Portfolios", expanded=False):
        ...
        # Initialize session state for portfolio checkboxes
        if "portfolio_checkboxes" not in st.session_state:
            st.session_state.portfolio_checkboxes = {}
        
        # Clean up any checkboxes for portfolios that no longer exist
        current_portfolio_names = set(portfolio_names)
        existing_checkbox_names = set(st.session_state.portfolio_checkboxes.keys())
        for old_name in existing_checkbox_names - current_portfolio_names:
            del st.session_state.portfolio_checkboxes[old_name]
        ...
        with col1:
            if st.button("✅ Select All", key="select_all_portfolios", 
                        help="Select all portfolios for deletion", use_container_width=True):
                for name in portfolio_names:
                    st.session_state.portfolio_checkboxes[name] = True
                st.rerun()
        
        with col2:
            if st.button("❌ Clear All", key="clear_all_portfolios", 
                        help="Clear all portfolio selections", use_container_width=True):
                st.session_state.portfolio_checkboxes = {}
                st.rerun()
        
        with col3:
            if st.button("🔄 Refresh", key="refresh_selections", 
        ...
        for i, portfolio_name in enumerate(portfolio_names):
            ...
            if portfolio_name not in st.session_state.portfolio_checkboxes:
                st.session_state.portfolio_checkboxes[portfolio_name] = False
            
            # Create a unique callback function for each portfolio
            def create_portfolio_callback(portfolio_name):
                def callback():
                    # Toggle the current state
                    current_state = st.session_state.portfolio_checkboxes.get(portfolio_name, False)
                    st.session_state.portfolio_checkboxes[portfolio_name] = not current_state
                return callback
            
            # Create checkbox for each portfolio with callback
            checkbox_key = f"portfolio_checkbox_{hash(portfolio_name)}"
            is_checked = st.checkbox(
                f"🗑️ {portfolio_name}",
                value=st.session_state.portfolio_checkboxes[portfolio_name],
                key=checkbox_key,
        ...
        selected_portfolios_for_deletion = [
            name for name, checked in st.session_state.portfolio_checkboxes.items() 
            if checked
        ]
        
        # Show success message if portfolios were deleted
        if "bulk_delete_success" in st.session_state and st.session_state.bulk_delete_success:
            st.success(st.session_state.bulk_delete_success)
            # Clear the success message after showing it
            del st.session_state.bulk_delete_success
    ...
    **💡 Tips**: 
    ...
    if use_momentum_vary:
        ...
        if enable_max_allocation:
            ...
            # Add/Remove buttons
            col_add_max, col_remove_max = st.columns([0.8, 0.2])
            with col_add_max:
            ...
            for i, max_allocation_value in enumerate(st.session_state[f"max_allocation_filters_{portfolio_index}"]):
                col_max_input, col_max_remove = st.columns([0.8, 0.2])
    ...
    if variant_params:
        ...
        if validation_errors:
            ...
            if st.button(f"✨ Generate {total_variants} Portfolio Variants", type="primary"):
                ...
                def generate_portfolio_variants(base_portfolio, variant_params, base_name):
                    ...
                    for i, combination in enumerate(combinations):
                        ...
                        for param in param_names:
                            if param in variant:
                                ...
                                if isinstance(value, bool):
                                    ...
                                    if param == "minimal_threshold" and value is not None:
                                        variant_name_parts.append(f"Threshold_{value}%")
                                    elif param == "minimal_threshold" and value is None:
                                        variant_name_parts.append("NoThreshold")
                                    elif param == "max_allocation" and value is not None:
                                        variant_name_parts.append(f"MaxAlloc_{value}%")
                                    elif param == "max_allocation" and value is None:
                                        variant_name_parts.append("NoMaxAlloc")
                                    else:
                        ...
                        if variant_name_parts:
                            variant['name'] = f"{base_name}_Variant_{i+1}_{'_'.join(variant_name_parts)}"
                        else:
                            variant['name'] = f"{base_name}_Variant_{i+1}"
                ...
                # CRITICAL: Copy the portfolio data BEFORE any operations to avoid reference issues
                ...
                # Generate variants BEFORE removing the original portfolio
                ...
                # NOW remove the original portfolio (AFTER generating variants)
                if not keep_current_portfolio:
                    if len(st.session_state.multi_backtest_portfolio_configs) > 1:
                ...
                # Add variants to portfolio list with unique names - simple approach
                for variant in variants:
                    # Use central function - automatically ensures unique name
...
with col_freq_rebal:
    ...
    st.selectbox("Rebalancing Frequency", freq_options, key="multi_backtest_active_rebal_freq", on_change=update_rebal_freq, help="How often the portfolio is rebalanced. 'Buy & Hold': When cash is available (from additions or dividends), it's immediately reinvested using the current portfolio proportions. 'Buy & Hold (Target)': When cash is available (from additions or dividends), it's immediately reinvested using the target allocations. Cash from dividends (if 'Collect Dividends as Cash' is enabled) will be available for rebalancing.", )
with col_freq_add:
    ...
    st.selectbox("Added Frequency", freq_options, key="multi_backtest_active_add_freq", on_change=update_add_freq, help="How often cash is added to the portfolio. 'Buy & Hold': When cash is available (from additions or dividends), it's immediately reinvested using the current portfolio proportions. 'Buy & Hold (Target)': When cash is available (from additions or dividends), it's immediately reinvested using the target allocations.")
...
# Smart benchmark handling - only update when portfolio actually changes
benchmark_key = "multi_backtest_active_benchmark"
benchmark_portfolio_tracker_key = "multi_backtest_benchmark_portfolio_tracker"

# Check if we need to update the benchmark (new portfolio or first time)
if (benchmark_key not in st.session_state or 
    benchmark_portfolio_tracker_key not in st.session_state or
    st.session_state[benchmark_portfolio_tracker_key] != st.session_state.multi_backtest_active_portfolio_index):
    
    # Portfolio changed or first time - update benchmark
    st.session_state[benchmark_key] = active_portfolio['benchmark_ticker']
    st.session_state[benchmark_portfolio_tracker_key] = st.session_state.multi_backtest_active_portfolio_index

st.text_input("Benchmark Ticker (default: ^GSPC, used for beta calculation)", key=benchmark_key, on_change=update_benchmark)
...
if st.session_state.get('multi_backtest_active_use_momentum', active_portfolio.get('use_momentum', True)):
    ...
    # Initialize threshold settings - ALWAYS sync with active portfolio
    st.session_state["multi_backtest_active_use_threshold"] = active_portfolio.get('use_minimal_threshold', False)
    st.session_state["multi_backtest_active_threshold_percent"] = active_portfolio.get('minimal_threshold_percent', 2.0)
...
with st.expander("JSON Configuration (Copy & Paste)", expanded=False):
    ...
    # Update threshold settings from session state
    cleaned_config['use_minimal_threshold'] = st.session_state.get('multi_backtest_active_use_threshold', False)
    cleaned_config['minimal_threshold_percent'] = st.session_state.get('multi_backtest_active_threshold_percent', 2.0)
        ...
        with contextlib.redirect_stdout(buffer):
            ...
            if not data:
                ...
                for i, cfg in enumerate(st.session_state.multi_backtest_portfolio_configs, start=1):
                    ...
                    # Apply minimal threshold filter if enabled
                    use_threshold = cfg.get('use_minimal_threshold', False)
                    threshold_percent = cfg.get('minimal_threshold_percent', 2.0)
                    
                    if use_threshold and today_weights_map:
                        # Apply threshold filter to today_weights_map
                        threshold_decimal = threshold_percent / 100.0
                        
                        # First: Filter out stocks below threshold (exclude CASH from filtering)
                        filtered_weights = {}
                        for ticker, weight in today_weights_map.items():
                            if ticker == 'CASH':
                                # Always keep CASH
                                filtered_weights[ticker] = weight
                            elif weight >= threshold_decimal:
                                # Keep stocks above or equal to threshold
                                filtered_weights[ticker] = weight
                        
                        # Then: Normalize remaining non-cash stocks to sum to (1 - cash_weight)
                        if filtered_weights:
                            cash_weight = filtered_weights.get('CASH', 0)
                            non_cash_weights = {k: v for k, v in filtered_weights.items() if k != 'CASH'}
                            
                            if non_cash_weights:
                                total_non_cash = sum(non_cash_weights.values())
                                if total_non_cash > 0:
                                    target_non_cash = 1.0 - cash_weight
                                    normalized_non_cash = {k: (v / total_non_cash) * target_non_cash for k, v in non_cash_weights.items()}
                                    
                                    # Reconstruct the weights map
                                    today_weights_map = normalized_non_cash.copy()
                                    if cash_weight > 0:
                                        today_weights_map['CASH'] = cash_weight
                                else:
                                    # If no non-cash stocks meet threshold, keep original
    ...
    try:
        ...
        if isinstance(obj, list):
            for portfolio in obj:
                # Add missing fields with default values (only if they don't exist)
        ...
        if isinstance(obj, list):
            ...
            if processed_configs:
                ...
                # Update benchmark portfolio tracker after import
...
with st.sidebar.expander('All Portfolios JSON (Export / Import)', expanded=False):
    ...
    # Test if callback function exists and can be called
    try:
        st.button('Update All Portfolios from JSON', on_click=paste_all_json_callback)
    except Exception as e:
        st.error(f"Error with callback function: {e}")
                        ...
                        try:
                            ...
                            if len(alloc_dates) == 0:
                                ...
                                try:
                                    ...
                                    if last_rebal_date_for_timer and rebalancing_frequency != 'none':
                                        ...
                                        if next_date and time_until:
                                            # Add spacing before the timer section to ensure proper positioning
                                            ...
                                            # Add proper spacing after the timer section to prevent page breaks
                                ...
                                with col_plot1:
                                    ...
                                    fig_rebal.update_layout(template='plotly_dark', height=400)
                                ...
                                with col_plot2:
                                    ...
                                    fig_final.update_layout(template='plotly_dark', height=400)
                    ...
                    if not allocs_for_portfolio:
                        ...
                        try:
                            ...
                            with col_plot1:
                                ...
                                fig_rebal.update_traces(textinfo='percent+label')
                                fig_rebal.update_layout(template='plotly_dark')
                            ...
                            with col_plot2:
                                ...
                                fig_final.add_trace(go.Pie(labels=labels_final, values=vals_final, hole=0.3))
                                fig_final.update_traces(textinfo='percent+label')
                                fig_final.update_layout(template='plotly_dark')

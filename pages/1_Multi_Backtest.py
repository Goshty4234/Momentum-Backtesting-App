        ...
        if validation_errors:
            ...
            if st.button(f"✨ Generate {total_variants} Portfolio Variants", type="primary"):
                ...
                if original_keep_current:
                    ...
                    st.session_state[f"success_message_{portfolio_index}"] = f"🎉 **Generated {len(variants)} variants** of '{base_name}'! Original portfolio removed."
                    st.session_state[f"info_message_{portfolio_index}"] = f"📊 Total portfolios: {len(st.session_state.multi_backtest_portfolio_configs)}"
...
if 'multi_backtest_ran' in st.session_state and st.session_state.multi_backtest_ran:
    if 'multi_all_results' in st.session_state and st.session_state.multi_all_results:
        ...
        for name, series_dict in st.session_state.multi_all_results.items():
            try:
                ...
                fig1.add_trace(go.Scatter(
                    ...
                    mode='lines', 
                    name=name,
                    hovertemplate=f"<b>{name}</b><br>Portfolio Value: $%{{y:,.2f}}<br>Date: %{{x|%Y-%m-%d}}<extra></extra>",
                    hoverinfo='text'
                ))
            except Exception as e:
        ...
        for name, series_dict in st.session_state.multi_all_results.items():
            try:
                ...
                fig2.add_trace(go.Scatter(
                    ...
                    mode='lines', 
                    name=name,
                    hovertemplate=f"<b>{name}</b><br>Drawdown: %{{y:.2f}}%<br>Date: %{{x|%Y-%m-%d}}<extra></extra>",
                    hoverinfo='text'
                ))
            except Exception as e:
        ...
        fig2.update_layout(
            ...
            margin=dict(l=80, r=80, t=80, b=120),  # More space at bottom for legend
            height=600,  # Taller height to prevent crushing
            yaxis=dict(
                title="Drawdown (%)", 
                title_standoff=20,
                        ...
                        if ticker in alloc_df.columns:
                            ...
                            if not ticker_data.empty:  # Only add if we have data
                                ...
                                fig_evolution.add_trace(go.Scatter(
                                    ...
                                    name=ticker,
                                    line=dict(color=colors[i % len(colors)], width=2),
                                    hovertemplate=f"<b>{ticker}</b><br>Allocation: %{{y:.1f}}%<br>Date: %{{x|%Y-%m-%d}}<extra></extra>",
                                    hoverinfo='text'
                                ))
